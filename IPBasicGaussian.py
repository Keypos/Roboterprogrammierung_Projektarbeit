...
class _simple_Gaussian(GaussianBase.C_GaussianBase):
    ...
    def _learnRoadmapNearestNeighbour(self, radius, numNodes):
        ...
        while nodeID <= numNodes:
            ...
            newNodePos = False
            while newNodePos == False:
                newNodePos = self.simple_Gaus_Sampling(self._collisionChecker)
    ...
    def planPath(self, startList, goalList, config):
        ...
        for node in result:
            if not self._collisionChecker.lineInCollision(checkedStartList[0], node[1]['pos']):
                self.graph.add_node(
                    "start", pos=checkedStartList[0], color='lightgreen')
        ...
        for node in result:
            if not self._collisionChecker.lineInCollision(checkedGoalList[0], node[1]['pos']):
                self.graph.add_node(
                    "goal", pos=checkedGoalList[0], color='lightgreen')
