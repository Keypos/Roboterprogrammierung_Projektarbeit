...
class GaussianPRM(IPPRMBase.PRMBase):
    ...
    def _learnRoadmapNearestNeighbour(self, dim, radius, numNodes):
        ...
        while i < numNodes:
            ...
            result = self._nearestNeighboursXX(pos, self._graph, radius)
    ...
    def _sample(self):
        limits = self._collChecker.getEnvironmentLimits()        
        pos = [random.uniform(limit[0],limit[1]) for limit in limits]
        
        
        #get a colliding point
        for t in range(0,10):
            while not self._collChecker.pointInCollision(pos):
                pos = [random.uniform(limit[0],limit[1]) for limit in limits]
            ...
            #get a distance over a gaussian distribution   
                ...
                for n in range(8):
                    ...
                    for alpha in new_angles_list:    
                        ...
                        pos2=[pos2_x,pos2_y]
                        if not self._collChecker.pointInCollision(pos2): #return point when collision free
                            return pos2
